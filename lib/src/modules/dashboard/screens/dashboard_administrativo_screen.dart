...
class DashboardAdministrativoScreen extends StatefulWidget {
  const DashboardAdministrativoScreen({Key? key}) : super(key: key);

  @override
  State<DashboardAdministrativoScreen> createState() =>
      _DashboardAdministrativoScreenState();
}

class _DashboardAdministrativoScreenState
    extends State<DashboardAdministrativoScreen> {
  @override
  void initState() {
    super.initState();
  ...
  Widget _buildTopSocios(DashboardProvider provider) {
    ...
    return TopSociosWidget(socios: socios);
  }
...
// Widget separado para manejar la paginación de Top Socios
class TopSociosWidget extends StatefulWidget {
  final List<TopSocio> socios;

  const TopSociosWidget({Key? key, required this.socios}) : super(key: key);

  @override
  State<TopSociosWidget> createState() => _TopSociosWidgetState();
}

class _TopSociosWidgetState extends State<TopSociosWidget> {
  int currentPage = 1;
  int itemsPerPage = 5;

  @override
  Widget build(BuildContext context) {
    final totalPages = (widget.socios.length / itemsPerPage).ceil();
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          if (widget.socios.isNotEmpty) ...[
            ...widget.socios
                .skip((currentPage - 1) * itemsPerPage)
                .take(itemsPerPage)
                .map((socio) => _buildSocioItem(socio))
                .toList(),

            // Controles de paginación simples si hay más de una página
            if (totalPages > 1) ...[
              const SizedBox(height: 16),
              Row(
                ...
                children: [
                  ...
                  IconButton(
                    ...
                    color:
                        currentPage > 1 ? Colors.orange : Colors.grey.shade400,
                    iconSize: 24,
                  ),
